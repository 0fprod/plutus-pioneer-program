...
payContract = do
    ...
    handleError (\err -> Contract.logError $ "caught: " ++ unpack err) $ void $ submitTx tx -- Handle the error so the contract doesnt crash and the second payment can be made
    payContract
...
payTrace firstPayment secondPayment = do 
    ...
    callEndpoint @"pay" h1 $ PayParams {
        ...
        ppLovelace = firstPayment
    }
    void $ Emulator.waitNSlots 1
    callEndpoint @"pay" h1 $ PayParams {
        ppRecipient = mockWalletPaymentPubKeyHash $ knownWallet 2,
        ppLovelace = secondPayment
    }
    void $ Emulator.waitNSlots 1
